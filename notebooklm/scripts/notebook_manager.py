...
def main():
    ...
    # Sync command - sync with NotebookLM API
    subparsers.add_parser('sync', help='Sync local library with NotebookLM API')
        ...
        async def do_sync():
            try:
                ...
                async with await NotebookLMClient.from_storage() as client:
                    api_notebooks = await client.notebooks.list()
                    print(f"\nNotebookLM API: {len(api_notebooks)} notebooks")

                    for nb in api_notebooks:
                        print(f"  {nb.id}: {nb.title} ({nb.sources_count} sources)")

                    # Update local library with API IDs
                    local_notebooks = library.list_notebooks()
                    updated = 0
                    added = 0

                    for api_nb in api_notebooks:
                        api_url = notebook_url_from_id(api_nb.id)
                        # Find matching local notebook by URL
                        found = False
                        for local_nb in local_notebooks:
                            if api_nb.id in local_nb.get('url', '') or local_nb.get('notebooklm_id') == api_nb.id:
                                # Update notebooklm_id
                                if not local_nb.get('notebooklm_id'):
                                    local_nb['notebooklm_id'] = api_nb.id
                                    library.notebooks[local_nb['id']] = local_nb
                                    updated += 1
                                found = True
                                break
                        ...
                        if not found:
                            ...
                            try:
                                library.add_notebook(
                                    url=api_url,
                                    name=api_nb.title or f"Notebook {api_nb.id[:8]}",
                                    description=f"Synced from NotebookLM ({api_nb.sources_count} sources)",
                                    topics=["synced"],
                                    notebooklm_id=api_nb.id,
                                )
                                added += 1
                            except ValueError:
                                pass  # duplicate ID
